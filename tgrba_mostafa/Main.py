...
#     dig.bill.addItem("طيور المصطفى\t٠١٠٩٣٨٨٩٣٣١,٠١١٢٧٧٨١٣٥٦")
...
#                 dig.bill.addItem("{}\t{}\t{} × {} =\t{} ".format(str(qun), prod, str(qun), price, str(qun * float(price))))
#                 dig.screen.clear()
#                 self.z.append([qun, prod, int(qun) * int(price), billnum])
#                 self.quant = ""
#         except Exception as e:
#             print(e)
#
...
#             # painter.setLayoutDirection(Qt.AlignRight)
#             painter.setFont(QFont("Segoe UI", 9, 500))
#             painter.drawText(240,0,0, 2000, Qt.TextIncludeTrailingSpaces | Qt.AlignRight, bill)
#             painter.end()
#         except Exception as e:
#             print(e)
#
...
#             if not listItems:
#                 return
#             if dig.bill.currentRow() not in(0,1,2):
...
#             elif sender == "دين":
#                 c.execute("""insert into dyon (billnum,bill,total,[date],[time],notes,saler) values (?,?,?,?,?,?,?);""",
#                           (billnum, x, self.total, date, time, notes, self.saler))
#                 conn.commit()
#
#
#             for qun, prod, price, billnum in self.z:
#                 c.execute("""insert into sales (quantity,product,price,billnum,[date],[time])values (?,?,?,?,?,?)""",
...
#             conn = sqlite3.connect("m7l2.db")
#             c = conn.cursor()
#             c.execute("""DELETE FROm bills where billnum = (?) and bill =(?) and total=(?) and date=(?) and time=(?)""",
#                       (billnum, bill, total, date, time))
#             conn.commit()
#
#             c.execute("""DELETE FROm dyon where billnum = (?) and bill =(?) and total=(?) and date=(?) and time=(?)""",
#                       (billnum, bill, total, date, time))
#             conn.commit()
#
